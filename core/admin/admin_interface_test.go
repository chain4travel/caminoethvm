...
var (
	...
	*/
	testContractAbi = "[{\"inputs\":[],\"name\":\"bar\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"foo\",\"outputs\":[],\"stateMutability\":\"pure\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"account\",\"type\":\"address\"},{\"internalType\":\"bytes4\",\"name\":\"signature\",\"type\":\"bytes4\"}],\"name\":\"pack\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"pure\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"testSave\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"}]"
	testContractBin = ""
)

func deployContract(t *testing.T, backend *backends.SimulatedBackend, contractAbi string, contractBin string) (common.Address, error) {
...
func TestDeployUnverified(t *testing.T) {
	...
	defer backend.Close()
	_, err := deployContract(t, backend, testContractAbi, testContractBin)
	if err == nil {
...
func TestFucntionDirectExecution(t *testing.T) {
	for _, test := range []struct {
		...
		blacklistSelector := crypto.Keccak256([]byte(test.functionToCall))[:4]

		test1ContractAddr, err := deployContract(t, backend, testContractAbi, testContractBin)
		assert.NoError(t, err, "failed to deploy contract 1")

		blacklistFunction(t, backend, test1ContractAddr, blacklistSelector)

		backend.Commit(true)

		head, _ := backend.HeaderByNumber(ctx, nil) // Should be child's, good enough
		gasPrice := new(big.Int).Add(head.BaseFee, big.NewInt(1))
		tx := types.NewTransaction(3, test1ContractAddr, common.Big0, 3000000, gasPrice, blacklistSelector[:])
...
func TestDisabledContract(t *testing.T) {
	backend := backends.NewSimulatedBackendWithKYCVerified(
		core.GenesisAlloc{
			testAddr: {Balance: new(big.Int).Mul(big.NewInt(10000000000000000), big.NewInt(1000))},
		},
	...
	blacklistSelector := admin.DisableContractFunctionSelector[:4]
	...
	for i, functionToCall := range []string{
		"foo()",
		"bar()",
		"testSave()",
	} {
		...
		head, _ := backend.HeaderByNumber(ctx, nil) // Should be child's, good enough
		gasPrice := new(big.Int).Add(head.BaseFee, big.NewInt(1))
		tx := types.NewTransaction(3+uint64(i), test1ContractAddr, common.Big0, 3000000, gasPrice, callSelector[:])
		signer := types.NewLondonSigner(big.NewInt(1337))
		tx, _ = types.SignTx(tx, signer, testKey)
