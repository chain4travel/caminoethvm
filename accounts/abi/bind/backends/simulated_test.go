...
func simTestBackend(testAddr common.Address) *SimulatedBackend {
	return NewSimulatedBackendWithKYCVerified(
		core.GenesisAlloc{
...
func TestEstimateGas(t *testing.T) {
	...
	sim := NewSimulatedBackendWithKYCVerified(core.GenesisAlloc{addr: {Balance: big.NewInt(params.Ether)}}, 10000000, addr)
	defer sim.Close()

	parsed, _ := abi.JSON(strings.NewReader(contractAbi))
	contractAddr, _, _, err := bind.DeployContract(opts, parsed, common.FromHex(contractBin), sim)
	if err != nil {
		t.Fatalf("failed to create testing contract: %v", err)
	}
	sim.Commit(false)
