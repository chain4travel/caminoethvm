...
var (
	...
	sunrisePhase0BlockValidator = blockValidatorSunrisePhase0{}
)
...
type blockValidatorSunrisePhase0 struct{}

func (blockValidatorSunrisePhase0) SyntacticVerify(b *Block) error {
	if b == nil || b.ethBlock == nil {
		return errInvalidBlock
	}
	...
	if headerExtraDataSize := len(ethHeader.Extra); headerExtraDataSize != params.SunrisePhase0ExtraDataSize {
		return fmt.Errorf(
			"expected header ExtraData to be %d but got %d: %w",
			params.SunrisePhase0ExtraDataSize, headerExtraDataSize, errHeaderExtraDataTooBig,
		)
	}
	if ethHeader.BaseFee == nil {
		return errNilBaseFeeApricotPhase3
